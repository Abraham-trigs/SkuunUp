...
interface LibraryStaffState {
  ...
  setSelectedStaff: (staff: LibraryStaff | null) => void;
  ...
  return {
    ...
    fetchStaff: async (page = get().page, search = get().search) => {
      ...
      try {
        const data = await apiClient<{
          ...
          `/api/library/staff?search=${encodeURIComponent(search)}&page=${page}&perPage=${get().perPage}`
        );
        set((state) => ({
          ...
          cache:
            search === ""
              ? { ...state.cache, [page]: data.staffList }
              : state.cache,
        }));
      } catch (err: any) {
    ...
    fetchStaffById: async (id) => {
      ...
      try {
        ...
        const fetched = await apiClient<LibraryStaff>(`/api/library/staff/${id}`);
        set({
          selectedStaff: fetched,
          staffList: [fetched, ...get().staffList.filter((s) => s.id !== id)],
        });
        return fetched;
      } catch (err: any) {
    ...
    createStaff: async (payload) => {
      set({ loading: true, error: null });
      try {
        const staff = await apiClient<LibraryStaff>("/api/library/staff", {
          method: "POST",
          body: JSON.stringify(payload),
        });
        set((state) => ({
          staffList: [staff, ...state.staffList],
          total: state.total + 1,
        }));
        return staff;
      } catch (err: any) {
    ...
    deleteStaff: async (id, onDeleted) => {
      set({ loading: true, error: null });
      try {
        const res = await apiClient<{ success: boolean; error?: any }>(
          `/api/library/staff/${id}`,
          { method: "DELETE" }
        );
        if (res.success) {
          set((state) => ({
        ...
        } else set({ error: res.error?.message || "Failed" });
      } catch (err: any) {
        set({ error: err?.message || "Failed" });
      } finally {
        set({ loading: false });
